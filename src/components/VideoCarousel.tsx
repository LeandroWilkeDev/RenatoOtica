...
export default function VideoCarousel() {
  ...
  // Guarda refs dos <video> para controlar play/pause sem re-render desnecessario.
  ...
  // Garante que animacoes GSAP sejam limpas junto com o ciclo de vida do componente.
  ...
  const updatePositions = contextSafe((index: number) => {
    ...
    videos.forEach((_, i) => {
      ...
      // Calcula a distancia mais curta em um carrossel circular.
      let diff = i - index;
  ...
  // Recalcula posicoes ao trocar o indice ativo.
  useGSAP(() => {
    updatePositions(currentIndex);
  }, [currentIndex]);

  // Controla reproducao apenas quando a area do carrossel esta visivel.
  useGSAP(
    () => {
  ...
  // Mantem so o video central tocando; laterais ficam pausados e reiniciados.
  useEffect(() => {
    videos.forEach((_, i) => {
  ...
  const nextSlide = () => {
    setCurrentIndex((prev) => getLoopedIndex(prev + 1, videos.length));
  };
  ...
  return (
    <article className="reveal relative w-full mt-0 sm:mt-6 mb-8 transition-all flex flex-col items-center pt-16 sm:pt-0 pb-6 sm:py-6">
      ...
      <div
        ref={container}
        className="relative w-full h-[60dvh] lg:h-137.5 flex justify-center items-center overflow-hidden"
      >
        {videos.map((vid, i) => (
          <div
            key={i}
            className="video-slide absolute w-65 sm:w-75 aspect-9/16 rounded-[2.5rem] overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.8)] border border-white/20 bg-zinc-950 will-change-transform cursor-pointer"
